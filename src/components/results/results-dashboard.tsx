  ...
  if (!result) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 text-center">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">No analysis session found</p>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16 space-y-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/50 pb-4">
        <Link href="/analyze" className="inline-flex items-center gap-2 text-xs font-mono text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="size-3.5" />
          Modify Analysis Input
        </Link>
        <div className="inline-flex items-center gap-2 rounded-lg border border-border/70 bg-card/60 px-3 py-1.5 text-xs text-muted-foreground">
          <Info className="size-3.5" />
          <span>Analysis complete · {new Date(result.createdAt).toLocaleString()}</span>
        </div>
      </div>

      <RiskOverview
        score={result.overallRiskScore}
        blindSpotsCount={result.metrics?.blindSpots}
        assumptionsCount={result.metrics?.assumptions}
        dependenciesCount={result.metrics?.dependencies}
        evidenceCoverage={result.metrics?.evidenceCoverage}
      />

      <div className="rounded-xl border border-border/70 bg-card/40 p-4 text-xs leading-relaxed text-muted-foreground">
        <span className="text-xs font-semibold uppercase tracking-wider text-foreground">Analyzed input</span>
        <p className="mt-1 line-clamp-4 leading-relaxed">{result.systemDescription}</p>
      </div>

      <div className="flex flex-wrap items-center gap-2 border-y border-border/50 py-3 text-xs">
        <span className="text-muted-foreground mr-1 flex items-center gap-1.5 font-medium">
          <Layers className="size-3.5" />
          Sections:
        </span>
        <a href="#critical-blind-spots" className="rounded-md border border-border/60 bg-card/50 px-2.5 py-1 text-xs text-muted-foreground hover:border-border hover:bg-card/80 hover:text-foreground transition-all">
        ...
        {sectionMeta.map((section) => (
          <a
            key={section.id}
            href={`#${section.id}`}
            className="rounded-md border border-border/60 bg-card/50 px-2.5 py-1 text-xs text-muted-foreground hover:border-border hover:bg-card/80 hover:text-foreground transition-all"
          >
            {section.title}
          </a>
        ))}
      ...
      <section id="critical-blind-spots" className="space-y-4">
        <div className="border-b border-border/50 pb-3">
          <span className="text-xs font-semibold uppercase tracking-wider text-red-400">Catastrophic & High-Impact</span>
          <h2 className="mt-1 text-lg font-bold">Critical Blind Spots</h2>
          <p className="text-xs text-muted-foreground">Cross-category findings with the highest severity. This is a priority view, not a separate finding category.</p>
        </div>
        {critical.length ? (
          ...
          <p className="rounded-xl border border-border/70 bg-card/30 p-6 text-xs text-muted-foreground">
            No critical or high-severity findings were returned.
          </p>
        )}
      </section>
        ...
        {sectionMeta.map((section) => {
          ...
          return (
            <FindingsSection
              ...
              id={section.id}
              title={section.title}
              categoryTag={section.tag}
              description={section.description}
              count={items.length}
            >
              {items.map((finding: BlindSpotFinding) => (
                <FindingCard key={finding.id} finding={finding} />
              ))}
            </FindingsSection>
          );
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 rounded-xl border border-border/70 bg-card/50 p-6 text-center sm:text-left">
        <div>
          <h3 className="text-sm font-semibold">Ready to test another system?</h3>
          <p className="text-xs text-muted-foreground mt-0.5">Run a fresh adversarial audit with a different proposal or architecture.</p>
        </div>
        <Link href="/analyze" className={cn(buttonVariants({ variant: "outline", size: "sm" }), "text-xs")}>
